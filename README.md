...
The script validates the certificate, validates the private key, checks that they match, verifies that the certificate covers the OpenVAS FQDN or a valid wildcard SAN, copies the files to `/opt/openvas/certs/`, applies secure permissions, updates the Greenbone nginx TLS configuration, and verifies the certificate actually presented over HTTPS.
